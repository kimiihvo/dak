package study.spring.dak.helper;
...
public class WebHelper {
    ...
    public void init(HttpServletRequest request, HttpServletResponse response) {
    	...
    	String methodName = request.getMethod();			// GET 방식인지, POST 방식인지 조회한다.
    ...
    public void setUploadPath(String uploadPath) { this.uploadPath = uploadPath; }
	    ...
	    public UploadItem saveMultipartFile(MultipartFile multipartFile) throws NullPointerException, Exception {
			...
			// 절대경로에서 이미 root-context에 저장되어 있는 업로드 폴더 경로를 삭제한다.
			...
			//WebHelper에 의해 생성된 업로드 경로는 서버상의 위치일 뿐 웹상에 노출될 수 있는 형태가 아니다.
			//view를 통해 웹 상에 노출하기 위해서는 업로드 위치의 URL PATH를 덫붙인 형태로 경로를 가공해야 한다.
